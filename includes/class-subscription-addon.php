...
class WC_Addons_Gateway_Simplify_Commerce extends WC_Gateway_Simplify_Commerce {
	...
	public function __construct() {
		...
		if (class_exists('WC_Subscriptions_Order')) {
			...
			add_action(
				...
				array($this, 'update_failing_payment_method'),
				10,
				2
			);
			...
			add_filter(
				...
				array($this, 'add_subscription_payment_meta'),
				10,
				2
			);
			add_filter(
				'woocommerce_subscription_validate_payment_meta',
				array($this, 'validate_subscription_payment_meta'),
				10,
				2
			);
		}
	...
	public function process_subscription_payment($order, $amount = 0, $token = array()) {
		...
		if ('APPROVED' == $payment->paymentStatus) {
			...
			$order->add_order_note(sprintf(
				__('Gateway payment approved (ID: %s, Auth Code: %s)', 'woocommerce-gateway-simplify-commerce'),
				$payment->id,
				$payment->authCode
			));
	...
	public function process_pre_order_release_payment($order) {
		...
		try {
			...
			$pre_order_name = sprintf(
				__('%s - Pre-order for "%s"', 'woocommerce-gateway-simplify-commerce'),
				$order_item['name']
			) . ' ' . sprintf(
	...
	public function return_handler() {
		...
		if (isset($_REQUEST['reference']) && isset($_REQUEST['amount'])) {
			...
			$subscription = get_post_meta($order_id, 'subscription', true);

			if ($amount === $order_total) {
				if ($this->order_contains_subscription($order->get_id()) || ($subscription !== '' && $subscription !== null)) {
