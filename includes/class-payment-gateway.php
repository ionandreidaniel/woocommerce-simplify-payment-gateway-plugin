	...
	public function capture_authorized_order() {
		try {
			...
			if ($this->is_payment_already_captured($e)) {
				...
				wp_die(
					$e->getMessage() . '<br>Ref: ' . $e->getReference() . '<br>Code: ' . $e->getErrorCode(),
					__('Gateway Failure')
				);
			}
		} catch (Exception $e) {
	...
	public function void_authorized_order() {
		try {
			...
			$order->add_order_note(sprintf(
				__(
					'Gateway reverse authorization (ID: %s)',
					'woocommerce-gateway-simplify-commerce'
				),
				$authCode
			));
...
<?php
	...
	public function do_payment($order, $amount = 0, $token = array()) {
		...
		try {
			...
			$order->add_order_note(sprintf(
				__('Gateway payment error: %s', 'woocommerce-gateway-simplify-commerce'),
				$error_message
			));
		...
		if ('APPROVED' == $payment->paymentStatus) {
			...
			$order->add_order_note(
				sprintf(
					__(
						'Gateway payment approved (ID: %s, Auth Code: %s)',
						'woocommerce-gateway-simplify-commerce'
					),
					$payment->id,
					$payment->authCode
	...
	protected function get_modal_receipt_page(int $order_id) {
		...
		echo '<script src="https://www.simplify.com/commerce/simplify.pay.js"></script>';

		echo '<button class="btn btn-width btn-green" id="simplify-payment-button" ' . implode(' ', $button_args) . ' style="margin:0 0 20px!important;padding:0 20px!important;border:0!important;border-radius:10px!important;-webkit-border-radius:10px!important;background-color:transparent!important;color:#fff!important;font-family:Montserrat,sans-serif!important;font-size:16px!important;letter-spacing:0!important;cursor:pointer!important;">' . __('Plătește', 'woocommerce') . '</button>';

		// echo '<a class="btn btn-width" href="' . esc_url($order->get_cancel_order_url(wc_get_page_permalink('shop'))) . '">' . __('Renunță', 'woocommerce') . '</a>';
	}
	...
	public function process_authorization_order_status($order, $payment_id, $status, $auth_code, $is_capture = false) {
		if ('APPROVED' === $status) {
			...
			if ($is_capture) {
				...
				// Add order note
				$order->add_order_note(sprintf(__(
					'Gateway authorization approved (ID: %s, Auth Code: %s)',
					'woocommerce-gateway-simplify-commerce'
				), $payment_id, $auth_code));
			}
